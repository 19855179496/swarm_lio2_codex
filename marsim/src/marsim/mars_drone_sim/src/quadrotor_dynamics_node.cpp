...
int main(int argc, char** argv)
{
    ...
    n.param("print_status", print_status, false);
    n.param("status_log_hz", status_log_hz, 1.0);
    ...
    ros::Time last_log_time = ros::Time(0);
    while(n.ok())
    {
        ros::spinOnce();
        ...
        if (print_status && status_log_hz > 0.0) {
            if ((now_time - last_log_time).toSec() >= 1.0 / status_log_hz) {
