...
- 用收到的时间差 delta_t_quad 将队友的位姿从消息时刻向前推进到 lidar_end_time：
    - 位置按匀速模型更新：pos_end = pos + vel * dt。
    - 姿态用角速度的指数映射更新：rot_end = rot * Exp(gyr, dt)。

- 将推进后的位姿和时间打包进队友的 odom 时间队列 teammate_teammate_odom_time，用于之后的轨迹匹配（队列长度保持不超过 pos_num_in_traj）。
*/
...
#pragma omp parallel for
    ...
    *cluster_cloud = all_predict_region_cloud;
    ...
    clusters.clear();
    clusters = FEC::FastEuclideanClustering(cluster_cloud, min_high_inten_cluster_size, 0.5, 50);//0.3,50
...
#pragma omp parallel for
    ...
    *cluster_cloud = cluster_input_cloud;
    ...
    vector<pcl::PointIndices> clusters;
    clusters.clear();
    clusters = FEC::FastEuclideanClustering(cluster_cloud, min_high_inten_cluster_size, 0.5, 50);//0.3,1000
    // For every cluster...
    for (auto iter = clusters.begin(); iter != clusters.end(); iter++) {
        //For each point in current cluster...
        int cnt = 0;
...
void Multi_UAV::UpdateTracker(const double &lidar_end_time, const int &id, const Teammate &teammate,
    ...
    if (cluster_meas) {
        if (teammate.teammate_pos_in_body.norm() < 1e-3)
            return;
        ekf_meas.head(3) = state.rot_end * teammate.teammate_pos_in_body + state.pos_end;//转换到世界坐标系
        ekf_meas.tail(3) = Zero3d;//将速度测为 0，说明点云聚类不提供速度信息或在这里不信任速度
