...
double dynamic_filter_radius = 0.85;
...
void point_filter_teammate(const PointCloudXYZI &orig_pcl, PointCloudXYZI &pcl_out, const shared_ptr<Multi_UAV> &swarm_in) {
    ...
    for (int i = 0; i < orig_pcl.points.size(); i++) {
        ...
        for (auto iter = swarm_in->teammate_tracker.begin(); iter != swarm_in->teammate_tracker.end(); ++iter) {
            ...
            double filter_radius = dynamic_filter_radius;
            auto teammate_iter = swarm_in->teammates.find(teammate_id);

            if (teammate_iter != swarm_in->teammates.end() &&
                state.global_extrinsic_trans[teammate_id].norm() > 0.001 &&
                lidar_end_time - teammate_iter->second.last_connect_time < 2.0) {
                V3D teammate_pos_body = state.rot_end.transpose() *
                    (state.global_extrinsic_rot[teammate_id] * teammate_iter->second.teammate_state.pos_end +
                     state.global_extrinsic_trans[teammate_id] - state.pos_end);
                V3D dist = pt - teammate_pos_body;
                if (dist.norm() < filter_radius) {
                    is_dynamic = true;
                    continue;
                }
            }
        ...
        if (!is_dynamic) {
            for (int j = 0; j < swarm_in->temp_tracker.size(); ++j) {
                ...
                if (dist.norm() < dynamic_filter_radius)  {
                    is_dynamic = true;
                    continue;
                }
            }
...
int main(int argc, char **argv) {
    ...
    ros::NodeHandle nh("~");
    p_imu.reset(new ImuProcess());//初始化
    ...
    nh.param<double>("multiuav/dynamic_filter_radius", dynamic_filter_radius, 0.85);
...
//                cout << "---------------------------------------" <<  RESET <<  endl;
            ...
            for (auto iter = swarm->teammates.begin(); iter != swarm->teammates.end(); ++iter) {
                ...
                if (swarm->IsDurationShort(lidar_end_time, iter->second, id)) {
                    ...
                    //- 将推进后的位姿和时间打包进队友的 odom 时间队列 teammate_teammate_odom_time，
                    //用于之后的轨迹匹配（队列长度保持不超过 pos_num_in_traj）。
                    swarm->PropagateTeammateState(lidar_end_time, iter->second);
                    //EKF tracker prediction
                    //这段代码检查 id 对应的队友跟踪器（teammate_tracker）是否存在；若存在，则计算从该跟踪器上次更新时间到当前 lidar_end_time 的时间差 connect_interval；当
                    //该差值小于 2.0 秒时，调用该跟踪器的 predict(lidar_end_time) 做状态预测。换言之：只有当“失联”时间少于 2 秒，才做预测传播。
                    //预测队友的无人机位置与协方差矩阵
                    swarm->PredictTracker(lidar_end_time, id);
                }
            }

            //点云聚类，生成一簇一簇满足大小的点云并可视化
            ...
            //检测高反射率的的点，然后进行聚类。再与上面产生的簇进行比较，如果是同一个物体，那么就把is_high_intensity变成false
