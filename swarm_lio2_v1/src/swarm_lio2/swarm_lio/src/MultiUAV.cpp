...
Multi_UAV::Multi_UAV(const ros::NodeHandle &nh, const int & drone_id_) {
    ...
    LoadParam("multiuav/inten_threshold", inten_threshold, double(100));
    ...
    LoadParam("multiuav/min_traj_match_points", min_traj_match_points, int(50));
    LoadParam("multiuav/traj_time_tolerance", traj_time_tolerance, double(0.03));
    LoadParam("multiuav/traj_match_current_dist_thresh", traj_match_current_dist_thresh, double(0.75));
    LoadParam("multiuav/traj_match_max_error_thresh", traj_match_max_error_thresh, double(0.45));
    LoadParam("multiuav/observation_stale_time", observation_stale_time, double(0.8));
    LoadParam("multiuav/temp_tracker_max_miss_time", temp_tracker_max_miss_time, double(2.0));
    LoadParam("multiuav/temp_tracker_static_max_time", temp_tracker_static_max_time, double(3.0));
    LoadParam("multiuav/temp_tracker_static_grace_time", temp_tracker_static_grace_time, double(5.0));
    ...
    for (int i = 0; i < MAX_UAV_NUM; ++i) {
        ...
        initial_rot_in_uav0[i] = M3D::Identity();
    }
    has_initial_pos_in_uav0[0] = true;
    vector<double> init_pose_flat;
    if (nh_.getParam("multiuav/initial_poses_in_uav0", init_pose_flat)) {
        if (init_pose_flat.size() % 7 != 0) {
            cout << YELLOW << " -- [Initial Extrinsic Prior] Invalid initial pose list size: "
                 << init_pose_flat.size() << " (must be 7*N: [id,x,y,z,roll_deg,pitch_deg,yaw_deg,...])" << RESET << endl;
        } else {
            for (size_t k = 0; k + 6 < init_pose_flat.size(); k += 7) {
                int id = int(init_pose_flat[k] + (init_pose_flat[k] >= 0.0 ? 0.5 : -0.5));
                if (id < 0 || id >= MAX_UAV_NUM) {
                    cout << YELLOW << " -- [Initial Extrinsic Prior] Skip invalid id: " << id << RESET << endl;
                    continue;
                }
                has_initial_pos_in_uav0[id] = true;
                initial_pos_in_uav0[id] = V3D(init_pose_flat[k + 1], init_pose_flat[k + 2], init_pose_flat[k + 3]);
                V3D rpy_rad(init_pose_flat[k + 4], init_pose_flat[k + 5], init_pose_flat[k + 6]);
                rpy_rad /= 57.3;
                initial_rot_in_uav0[id] = EulerToRotM(rpy_rad);
                cout << GREEN << " -- [Initial Extrinsic Prior] UAV" << id << " pose in UAV0, pos "
                     << initial_pos_in_uav0[id].transpose() << ", rpy_deg "
                     << init_pose_flat[k + 4] << " " << init_pose_flat[k + 5] << " " << init_pose_flat[k + 6]
                     << RESET << endl;
            }
        }
    }
    ...
    vector<double> init_rot_flat;
    if (nh_.getParam("multiuav/initial_rotations_deg_in_uav0", init_rot_flat)) {
        if (init_rot_flat.size() % 4 != 0) {
            cout << YELLOW << " -- [Initial Extrinsic Prior] Invalid initial rotation list size: "
                 << init_rot_flat.size() << " (must be 4*N: [id,roll_deg,pitch_deg,yaw_deg,...])" << RESET << endl;
        } else {
            for (size_t k = 0; k + 3 < init_rot_flat.size(); k += 4) {
                int id = int(init_rot_flat[k] + (init_rot_flat[k] >= 0.0 ? 0.5 : -0.5));
                if (id < 0 || id >= MAX_UAV_NUM) {
                    cout << YELLOW << " -- [Initial Extrinsic Prior] Skip invalid id: " << id << RESET << endl;
                    continue;
                }
                V3D rpy_rad(init_rot_flat[k + 1], init_rot_flat[k + 2], init_rot_flat[k + 3]);
                rpy_rad /= 57.3;
                initial_rot_in_uav0[id] = EulerToRotM(rpy_rad);
                cout << GREEN << " -- [Initial Extrinsic Prior] UAV" << id << " rot in UAV0 rpy_deg "
                     << init_rot_flat[k + 1] << " " << init_rot_flat[k + 2] << " " << init_rot_flat[k + 3]
                     << RESET << endl;
            }
        }
    }
    for (int id = 0; id < MAX_UAV_NUM; ++id) {
        ...
        vector<double> rot_rpy_deg;
        key = "multiuav/initial_rot_deg_uav" + SetString(id) + "_in_uav0";
        if (nh_.getParam(key, rot_rpy_deg)) {
            if (rot_rpy_deg.size() == 3) {
                V3D rpy_rad(rot_rpy_deg[0], rot_rpy_deg[1], rot_rpy_deg[2]);
                rpy_rad /= 57.3;
                initial_rot_in_uav0[id] = EulerToRotM(rpy_rad);
                cout << GREEN << " -- [Initial Extrinsic Prior] " << key << " = "
                     << rot_rpy_deg[0] << " " << rot_rpy_deg[1] << " " << rot_rpy_deg[2] << RESET << endl;
            } else {
                cout << YELLOW << " -- [Initial Extrinsic Prior] Invalid " << key
                     << " size: " << rot_rpy_deg.size() << " (must be 3)" << RESET << endl;
            }
        }
    }
    ...
    LoadParam("multiuav/same_obj_thresh", same_obj_thresh, same_obj_thresh);
    LoadParam("multiuav/valid_temp_cluster_dist_thresh", valid_temp_cluster_dist_thresh, valid_temp_cluster_dist_thresh);
...
bool Multi_UAV::GetInitialExtrinsicPrior(const int &id, M3D &prior_rot, V3D &prior_trans) {
    ...
    prior_rot = initial_rot_in_uav0[drone_id].transpose() * initial_rot_in_uav0[id];
    prior_trans = initial_rot_in_uav0[drone_id].transpose() *
                  (initial_pos_in_uav0[id] - initial_pos_in_uav0[drone_id]);
    return true;
}
...
void Multi_UAV::FuseEstimatedExtrinsicWithPrior(const int &id, const M3D &est_rot, const V3D &est_trans,
    ...
    double trans_diff = (est_trans - prior_trans).norm();
    M3D dR = prior_rot.transpose() * est_rot;
...
void Multi_UAV::UpdateFactorGraph(const bool &print_log) {
    ...
    if (use_initial_extrinsic_prior) {
        for (int id = 0; id < MAX_UAV_NUM; ++id) {
            ...
            double trans_diff = (state.global_extrinsic_trans[id] - prior_trans).norm();
            M3D dR_base_to_est = prior_rot.transpose() * state.global_extrinsic_rot[id];
            ...
            if (trans_diff > extrinsic_prior_trans_update_thresh || rot_diff_deg > extrinsic_prior_rot_update_thresh_deg) {
                ...
                if (print_log) {
                    cout << YELLOW << " -- [Initial Extrinsic Prior] Graph extrinsic for id " << id
                         << " exceeds prior threshold, use initial prior"
...
#pragma omp parallel for
    ...
    for (int i = 0; i < temp_tracker.size(); ++i) {
        ...
        if(kdtree_all->radiusSearch(searchPoint, temp_predict_region_radius, pointIdxRadiusSearch, pointRadiusSquaredDistance, 220) > 0){
    ...
    for (auto iter = teammate_tracker.begin(); iter != teammate_tracker.end(); ++iter) {
        ...
        vector<int> pointIdxRadiusSearch;//下标
        vector<float> pointRadiusSquaredDistance;//距离
        if(kdtree_all->radiusSearch(searchPoint, predict_region_radius, pointIdxRadiusSearch, pointRadiusSquaredDistance, 160) > 0){
            for (int j = 0; j < pointIdxRadiusSearch.size(); ++j){
                pcl::PointXYZ pt_add;
                pt_add.x = cur_pcl_undistort_remove_far_points->points[pointIdxRadiusSearch[j]].x;
                pt_add.y = cur_pcl_undistort_remove_far_points->points[pointIdxRadiusSearch[j]].y;
    ...
    *cluster_cloud = all_predict_region_cloud;
    ...
    clusters.clear();
    clusters = FEC::FastEuclideanClustering(cluster_cloud, min_cluster_size, 0.3, 200);
...
//        cout << YELLOW << " -- [High Intensity Detection] No valid points within 20m range" << RESET << endl;
//      return;
//  }
...
bool Multi_UAV::DeleteInvalidTemporaryTracker(const double &lidar_end_time, const int &index) {
    ...
    double motion_dist = DBL_MAX;
    if (!temp_tracker[index].dyn_pos_time.empty()) {
        motion_dist = (temp_tracker[index].dyn_tracker.get_state_pos() -
                       temp_tracker[index].dyn_pos_time.front().head(3)).norm();
    }
    bool still_finding_teammates = teammate_tracker.size() < actual_uav_num - 1;
    if (!still_finding_teammates &&
        exist_dt > temp_tracker_static_max_time &&
        exist_dt > temp_tracker_static_grace_time &&
        motion_dist < temp_tracker_min_motion) {
        Vector4d rejected_obj;
        rejected_obj.head(3) = temp_tracker[index].dyn_tracker.get_state_pos();
        rejected_obj(3) = lidar_end_time;
        rejected_static_objects.push_back(rejected_obj);
        cout << YELLOW << " -- [Delete Temporary Tracker " << index
             << "] Static object rejected, motion = " << motion_dist << " m" << RESET << endl;
        VisualizeTempTrackerDelete(lidar_end_time, index);
        temp_tracker.erase(temp_tracker.begin() + index);
        return true;
    }

    if (update_dt > temp_tracker_max_miss_time) {
...
bool Multi_UAV::TrajMatching(vector<Vector3d> &dyn_positions,
    ...
    double max_match_err = 0.0;
    for (size_t i = 0; i < dyn_positions.size(); ++i) {
        double match_err = (dyn_positions[i] - Rot * uav_positions[i] - trans).norm();
        total_match_err += match_err;
        max_match_err = max(max_match_err, match_err);
    }

    double ave_match_err = total_match_err / double(dyn_positions.size());
    cout << " -- [Trajectory Match] Average Matching Error: " << ave_match_err
         << ", Max Matching Error: " << max_match_err << endl;

    if (ave_match_err > ave_match_error_thresh)
        return false;
    if (max_match_err > traj_match_max_error_thresh)
        return false;
...
bool Multi_UAV::CreateTeammateTracker(const double &lidar_end_time, const int &index, StatesGroup &state_in,
    ...
    // ===== 添加调试日志 =====
    if (print_log) {
        cout << MAGENTA << " -- [Trajectory Matching] Attempting to match temp tracker " << temp_tracker[index].id 
             << " (trajectory length: " << dyn_pos_time.size() << " points)" << RESET << endl;
    }
    ...
    if (second_large_singular_value > traj_matching_start_thresh) {
        ...
        // ===== 添加轨迹激励足够的日志 =====
        if (print_log) {
            cout << GREEN << "    -- Trajectory excitation sufficient (singular value: " << second_large_singular_value << ")" << RESET << endl;
        }
        ...
        for (auto iter = teammates.begin(); iter != teammates.end(); ++iter) {   //每个队友飞机
            ...
            if (iter->second.teammate_odom_time.empty() ||
                ...
                for (int i = 0; i < iter->second.teammate_odom_time.size(); ++i) {  //队友的每个odom (200)
                    ...
                    if (!temp_dyn_pose.empty() &&
                        abs(temp_dyn_pose.front()(3) - iter->second.teammate_odom_time[i](3)) < traj_time_tolerance) {
                ...
                if (dyn_pos.size() < min_traj_match_points)
                    continue;
                //Trajectory Matching
                ...
                if (TrajMatching(dyn_pos, uav_pos, rot, trans, noise, id)) {
                    V3D matched_teammate_pos = rot * iter->second.teammate_state.pos_end + trans;
                    V3D tracked_object_pos = temp_tracker[index].dyn_tracker.get_state_pos();
                    double current_match_dist = (matched_teammate_pos - tracked_object_pos).norm();
                    if (current_match_dist > traj_match_current_dist_thresh) {
                        if (print_log) {
                            cout << YELLOW << " -- [Trajectory Matching] REJECTED current position mismatch: "
                                 << current_match_dist << " m > " << traj_match_current_dist_thresh << " m" << RESET << endl;
                        }
                        continue;
                    }
                                        ...
                                        // ===== 添加匹配成功日志 =====
                    if (print_log) {
                        cout << GREEN << " -- [Trajectory Matching] SUCCESS! Matched with teammate ID: " << id << RESET << endl;
                        cout << "    -- Relative rotation: " << RotMtoEuler(rot).transpose() * 57.3 << " deg" << endl;
                        cout << "    -- Relative translation: " << trans.transpose() << " m" << endl;
                        cout << BOLDGREEN << " -- [New Teammate Confirmed] UAV" << id << " is now a teammate!" << RESET << endl;
                    }
                    ...
                    if (!accepted_update) {
                        if (print_log) {
                            cout << YELLOW << " -- [Initial Extrinsic Prior] Reject teammate confirmation for id " << id
                                 << ": estimated extrinsic is far from initial prior." << RESET << endl;
                        }
                        continue;
                    }
                    ...
                    // ===== 添加匹配失败日志 =====
                    if (print_log) {
                        cout << YELLOW << " -- [Trajectory Matching] FAILED to match with teammate ID: " << id << RESET << endl;
                    }
                }
            }
        }
        ...
        // ===== 添加轨迹激励不足的日志 =====
        if (print_log) {
            cout << YELLOW << " -- [Trajectory Matching] Trajectory not excited enough (singular value: " << second_large_singular_value 
                 << " < " << traj_matching_start_thresh << ")" << RESET << endl;
        }
    }
    return false;
}
...
void Multi_UAV::CreateTempTrackerByHighIntensity(const double &lidar_end_time) {
    ...
    for (auto iter = rejected_static_objects.begin(); iter != rejected_static_objects.end();) {
        if (lidar_end_time - (*iter)(3) > static_reject_duration)
            iter = rejected_static_objects.erase(iter);
        else
            ++iter;
    }
    ...
    for (int i = 0; i < cluster_pos_tag.size(); i++) {
        if (cluster_pos_tag[i].is_high_intensity) {
            if (cluster_pos_tag[i].max_dist > valid_cluster_size_thresh)
                continue;
            ...
            for (const auto &rejected_obj : rejected_static_objects) {
                if ((teammate_pos_in_world - rejected_obj.head(3)).norm() < static_reject_radius) {
            ...
            bool tracked_by_temp = false;
            for (auto &tracker : temp_tracker) {
                if ((teammate_pos_in_world - tracker.dyn_tracker.get_state_pos()).norm() < same_obj_thresh) {
                    tracked_by_temp = true;
                    break;
                }
            }
            if (tracked_by_temp)
...
void Multi_UAV::UpdateTemporaryTracker(const double &lidar_end_time, const int &index, const bool &print_log) {
    double cluster_update_dt = lidar_end_time - temp_tracker[index].dyn_tracker.last_update_time_;
        ...
        // ===== 添加调试日志 =====
    if (print_log) {
        cout << CYAN << " -- [Temp Tracker " << temp_tracker[index].id << "] Updating..." << RESET << endl;
        cout << "    -- Current trajectory length: " << temp_tracker[index].dyn_pos_time.size() 
             << " / " << pos_num_in_traj << " points" << endl;
    }
    ...
    if (print_log && temp_tracker[index].exist_meas) {
        cout << GREEN << " -- [Temp Tracker " << temp_tracker[index].id << "] Updated with measurement, new length: "
             << temp_tracker[index].dyn_pos_time.size() + 1 << " points" << RESET << endl;
    } else if (print_log) {
        cout << YELLOW << " -- [Temp Tracker " << temp_tracker[index].id << "] No measurement this frame" << RESET << endl;
    }
...
void Multi_UAV::VisualizeTeammateTracker(const double &lidar_end_time, const int &teammate_id) {
    ...
    if (iter != teammate_tracker.end()) {
        ...
        auto iter_teammate = teammates.find(teammate_id);
        V3D vis_pos_world = tracker.get_state_pos();
        if (iter_teammate != teammates.end() &&
            state.global_extrinsic_trans[teammate_id].norm() > 0.001 &&
            lidar_end_time - iter_teammate->second.last_connect_time < 2.0) {
            vis_pos_world = state.global_extrinsic_rot[teammate_id] *
                            iter_teammate->second.teammate_state.pos_end +
                            state.global_extrinsic_trans[teammate_id];
        }
        V3D vis_pos_gravity = rot_world_to_gravity * vis_pos_world;
        M3D teammate_rot_end = M3D::Identity();
        if (iter_teammate != teammates.end())
            teammate_rot_end = iter_teammate->second.teammate_state.rot_end;
        M3D vis_rot_gravity = rot_world_to_gravity * state.global_extrinsic_rot[teammate_id] * teammate_rot_end;

        if(pubUAV.getNumSubscribers() < 1)
            return;

        //连续一段时间没有互观测就变黄；仿真中允许短时漏检，避免视觉状态抖动。
        V3D vis_color = Zero3d;
        if (lidar_end_time - tracker.last_update_time_ > observation_stale_time)
...
void Multi_UAV::VisualizeMeshUAV(const double &lidar_end_time, const int &teammate_id, const Teammate &teammate){
    ...
    if (iter != teammate_tracker.end()) {
        ...
        V3D vis_pos_world = tracker.get_state_pos();
        if (state.global_extrinsic_trans[teammate_id].norm() > 0.001 &&
            lidar_end_time - teammate.last_connect_time < 2.0) {
            vis_pos_world = state.global_extrinsic_rot[teammate_id] *
                            teammate.teammate_state.pos_end +
                            state.global_extrinsic_trans[teammate_id];
        }
...
void Multi_UAV::VisualizePredictRegion(const double &lidar_end_time, const int &id) {
    ...
    if (iter != teammate_tracker.end()) {
        ...
        if (teammate_iter != teammates.end() &&
            ...
            lidar_end_time - teammate_iter->second.last_connect_time < 2.0) {
            predict_pos_world = state.global_extrinsic_rot[id] *
                                teammate_iter->second.teammate_state.pos_end +
                                state.global_extrinsic_trans[id];
        }
        V3D predict_pos_in_gravity = rot_world_to_gravity * predict_pos_world;
        sphere.pose.position.x = predict_pos_in_gravity.x();
        sphere.pose.position.y = predict_pos_in_gravity.y();
        sphere.pose.position.z = predict_pos_in_gravity.z();
